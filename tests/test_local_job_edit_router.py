...
def _patch_common(monkeypatch, job):
    monkeypatch.setattr(local_job_edit, "get_job", lambda task_id, client_id=None: job)
    monkeypatch.setattr(local_job_edit, "_attach_result_artifacts", lambda task_id, result: result)
    monkeypatch.setattr(
...
def test_transcript_missing_job_returns_404(monkeypatch):
    monkeypatch.setattr(local_job_edit, "get_job", lambda task_id, client_id=None: None)
...
def test_transcript_edit_persists_scoped(monkeypatch):
    ...
    monkeypatch.setattr(local_job_edit, "get_job", scoped_get)
    monkeypatch.setattr(local_job_edit, "_sanitize_edit_segments", lambda value: [])
    monkeypatch.setattr(local_job_edit, "_sanitize_edit_records", lambda value: [])
    monkeypatch.setattr(local_job_edit, "_canonical_display_segments", lambda result: [])
    monkeypatch.setattr(local_job_edit, "_write_edited_transcript_backup", lambda task_id, result: Path("/tmp/edited.txt"))
    monkeypatch.setattr(local_job_edit, "_write_transcript_edit_records_backup", lambda task_id, result, records: Path("/tmp/rec.json"))
    monkeypatch.setattr(local_job_edit, "_attach_result_artifacts", lambda task_id, result: result)
    monkeypatch.setattr(
        local_job_edit, "update_job_result",
...
def test_transcript_backup_failure_returns_500(monkeypatch):
    ...
    monkeypatch.setattr(local_job_edit, "_write_edited_transcript_backup", boom)
...
def test_local_job_edit_stays_off_server_helpers():
    source = Path("backend/routers/local_job_edit.py").read_text(encoding="utf-8")
